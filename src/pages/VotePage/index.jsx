            ...
            >
              <div className="w-1/2 relative px-5 py-6 flex flex-col justify-between">
                <div className="flex flex-col gap-y-5 w-full">
                  ...
                  <div className="text-white mt-2 mb-0.5 w-full text-center font-semibold text-base">
                    ...
                    {meetingDetails.UserId === currentUser.userId || meetingDetails.GetAllMeetingItemDto.$values.some(e => e.UserId === currentUser.userId && e.IsVoted) ? (
                      <>
                        ANKET SONUÇLARI
                      </>) :
                          ...
                          return (
                            <div
                              ...
                              className={classNames("w-full flex items-center justify-start gap-x-2 ", {
                                "justify-between": meetingDetails.UserId === currentUser.userId || meetingDetails.GetAllMeetingItemDto.$values.some(e => e.UserId === currentUser.userId && e.IsVoted)
                              })}
                            >
                              {meetingDetails.UserId === currentUser.userId || meetingDetails.GetAllMeetingItemDto.$values.some(e => e.UserId === currentUser.userId && e.IsVoted) ? (<></>) : (<>
                                <input
                              ...
                              {meetingDetails.UserId === currentUser.userId || meetingDetails.GetAllMeetingItemDto.$values.some(e => e.UserId === currentUser.userId && e.IsVoted) ? (<div>
                ...
                {meetingDetails.UserId === currentUser.userId || meetingDetails.GetAllMeetingItemDto.$values.some(e => e.UserId === currentUser.userId && e.IsVoted) ? (<></>) : (
                  <>
                    <Button
                      onClick={() => { meetingDetails.GetAllMeetingItemDto?.$values.some(e => e.UserId === currentUser.userId && !e.IsVoted) ? handleVote() : (undefined) }}
              ...
              <div className="flex-1 bg-[#1d2629]  p-4 rounded-tr-3xl border-l border-l-[#F7B22C] flex flex-col gap-y-3 overflow-auto">
                ...
                {meetingDetails.GetAllMeetingItemDto?.$values.map((person, i) => (
                  <div
                    className={classNames("mb-1 w-full py-1.5 rounded-full flex items-center justify-center bg-[#32414a] font-semibold", {
