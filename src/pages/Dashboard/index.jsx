...
import { GetAllMeetings } from "../../api/server";
...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await GetAllMeetings(currentUser.usertoken);
        setMeeting(response);
      } catch (error) {
        console.error(error);
      }
      ...
      <div className="dashboardleft w-[70%]">
        <div className="flex flex-col w-full justify-between px-2 gap-y-5">
          ...
          <div className="w-full flex flex-col text-[color:var(--color-base)]  gap-y-1">
            {/**KULLANICININ OLUŞTURDUĞU TOPLNATILAR */}
            {currentUser && meeting?.map((user, index) => {
              if (user.Email === currentUser.usermail) {
                return (
                  <div key={index} className="w-full bg-[color:var(--bg-base-secondary)] transition-all duration-500 rounded-md flex justify-between items-center px-4 py-2">
                    ...
                    <div className="text-center">{user.MeetingName}</div>
